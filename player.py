...
def create_beautiful_background():
    ...
    ####текст
    canvas.create_text(640, 50, text='🎵 ботаническая выгонка',
                       font=('Arial', 24, 'bold'),
                       fill='#424242')
...
volume_scale = Scale(root, from_=100, to_=0, orient=VERTICAL,command=volume)
volume_scale.set(70)
volume_scale.place_forget()#cкрыт изначально

volume_icon = Label(root, text='🔊',font=('TimesNewRoman',20))
volume_icon.place(x=735,y=645)
volume_icon.bind('<Enter>', lambda e: volume_scale.place(x=720, y=545))
volume_scale.bind('<Leave>', lambda e: volume_scale.place_forget())


###выбор треков###
song_list = Listbox(root,width=40,height=10)
song_list.place(x=950,y=25)
...
###орловские лейблы
status =    Label(root,text='Тут будет ваш трек',fg='#8B0000', font=('TimesNewRoman',16,'bold'))
status.place(x=70,y=610)

###перемотка трека
seek_bar = Scale(root, from_=0, to=100, orient=HORIZONTAL,command=seek)
